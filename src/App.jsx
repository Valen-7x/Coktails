import axios from "axios";
import { useState } from "react";
import Navbar from "./Navbar";
function App() {
  const [data, setData] = useState({ drinks: [] });
  // URL de la API de cócteles
  const apiUrl = "https://www.thecocktaildb.com/api/json/v1/1/search.php?f=a";
  axios
    .get(apiUrl)
    .then((response) => {
      setData(response.data);
      console.log(response.data);
    })
    .catch((error) => {
      console.error(
        "Error al realizar la solicitud a la API de cócteles:",
        error
      );
    });
  return (
    <>
      <div className="w-[100%] h-[130vh] ">
        <Navbar />

        <div className="hover:translate-x-10 h-[40vh] mt-[2rem] w-[100%] flex justify-center">
          <img className="h-[12rem]" src="https://www.creativefabrica.com/wp-content/uploads/2020/06/18/1592479206/Martini-black-580x386.jpg" alt="" />
         <b className=" text-center text-gradient-to-r from-indigo-500 text-xl text-brown w-[40vw] pt-[2rem]">
         Bienvenido a Cocktail's. Un sitio en el que encontrarás recetas de bebidas y cócteles de todo el mundo.
          También ofrecemos diversos datos de las mismas con fin informativo, ludico y educativo.
          Si le gusta el sitio, considere apoyarnos en todas nuestras plataformas.
         </b>
          </div>
        <div className="flex flex-col  gap-2 h-[95vh] items-center justify-center bg-[url('data:image/jpeg;base64,')] ">
        <button type="button" class="font-bold text-white mt-[1rem] bg-gradient-to-r from-gray-400 to-gray-700 hover:from-gray-500 hover:to-yellow-500 rounded-xl h-[5rem] w-[8rem]">
           Categories
           </button>
          <div className="flex flex-row gap-2 items-center">
          
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 5 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90 "
                  src={data.drinks[5].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[5]?.strIngredient1} </b>
            </div>
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[4].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[4]?.strIngredient1} </b>
            </div>
            <div />
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[3].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[3]?.strIngredient1} </b>
            </div>
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[2].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[2]?.strIngredient1} </b>
            </div>
          </div>
          <div className="flex flex-row gap-2 ">
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem]  rounded-2xl hover:scale-90"
                  src={data.drinks[1].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[1]?.strIngredient1} </b>
            </div>
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[0].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[0]?.strIngredient1} </b>
            </div>
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[6].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[6]?.strIngredient1} </b>
            </div>
            <div className="flex flex-col justify-center items-center">
              {data.drinks.length > 0 && (
                <img
                  className="w-[15rem] h-[15rem] rounded-2xl hover:scale-90"
                  src={data.drinks[7].strDrinkThumb}
                  alt=""
                />
              )}
              <b className="text-black"> {data.drinks[7]?.strIngredient1} </b>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default App;
